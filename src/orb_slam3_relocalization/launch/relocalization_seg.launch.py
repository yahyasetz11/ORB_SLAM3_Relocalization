...
def generate_launch_description():
    ...
    params_file = os.path.join(
        pkg_share, '..', '..', '..', '..',
        'src', 'orb_slam3_relocalization', 'config', 'relocalization_seg_params.yaml')
    ...
    return LaunchDescription([
        ...
        Node(
            package='yolo_bbox',
            executable='yolo_seg_video',
            name='yolo_seg_node',
            output='screen',
            parameters=[params_file],
        ),

        # ── relocalization_seg_node ───────────────────────────────────────────
        Node(
            package='orb_slam3_relocalization',
            executable='relocalization_seg_node',
            name='relocalization_seg_node',
            output='screen',
            parameters=[params_file],
        ),
    ])
