"""
Single-plot comparison: one Std PnP reference line + one WPnP line per CSV.

Useful when sweeping a WPnP parameter (e.g. bg-weight) across multiple runs.
All runs produce nearly identical Std PnP results, so only the first CSV's
Std PnP is drawn as a shared reference baseline.  Each CSV contributes one
solid WPnP line with its own colour.
...
Usage:
    ...
    # Skip Umeyama alignment
    python3 plot2.py --no-align
"""
...
STD_COLOR = "#555555"   # neutral grey for the single Std PnP baseline
...
def main():
    parser = argparse.ArgumentParser(description="Single-plot WPnP parameter sweep")
    ...
    if len(args.csv) != len(args.gt):
        sys.exit(f"--csv and --gt must have the same count")

    labels = args.labels or [p.split("/")[-1].replace(".csv", "") for p in args.csv]
    if len(labels) != len(args.csv):
        sys.exit(f"--labels count must match --csv count")
    ...
    # ── Single Std PnP baseline (first CSV only) ──────────────────────────────
    ref = results[0]
    ts_std = ref["timestamps_std"] - t0
    ax.plot(
        ts_std, ref["std_err"],
        color=STD_COLOR, linewidth=1.2, alpha=0.7, linestyle="-",
        label=f"Std PnP (ref)  mean={ref['std_err'].mean():.3f}m",
        zorder=2,
    )
    ax.axhline(ref["std_err"].mean(), color=STD_COLOR, linestyle="--",
               linewidth=0.8, alpha=0.35, zorder=1)

    # ── One WPnP line per CSV ─────────────────────────────────────────────────
    for i, r in enumerate(results):
        ...
        ax.plot(
            ...
            label=f"{r['label']}  mean={r['wpnp_err'].mean():.3f}m",
            zorder=3,
        )
    ...
    ax.set_title(
        f"WPnP parameter sweep vs TUM ground truth{align_label}\n"
        f"Std PnP baseline (grey) from {labels[0]}"
    )
